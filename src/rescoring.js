...
const LicenseOverwriteFields = ({
  ...
  const [licensesFeature, setLicensesFeature] = React.useState()

  React.useEffect(() => {
    return registerCallbackHandler({
      featureRegistrationContext: featureRegistrationContext,
      featureName: features.LICENSES,
      callback: ({feature}) => setLicensesFeature(feature),
    })
  }, [featureRegistrationContext])

  const allLicenses = licensesFeature?.isAvailable ? licensesFeature.licenses : []
  const sortedLicenses = [...currentLicenses].sort((a, b) => a.name.localeCompare(b.name))

  const unknownLicense = licenseTo && !allLicenses.includes(licenseTo) ? 'Must be a known license' : null
  ...
  }, [licenseFrom, licenseTo, licensesFeature])
...
const OverwriteDialog = ({
  ...
  const overwriteFieldsForType = {
    [SCANNER_WRITEBACK_TYPES.LICENSE]: <LicenseOverwriteFields
    ...
    />,
    [SCANNER_WRITEBACK_TYPES.PACKAGE_VERSION]: <PackageVersionOverwriteFields
  ...
  >
    ...
    <DialogContent>
      {
        overwriteFieldsForType[scannerWritebackType]
      }
    </DialogContent>
    <DialogActions>
